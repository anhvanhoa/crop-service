...
func (s *PlantingCycleService) GetOverdueHarvests(ctx context.Context, req *emptypb.Empty) (*plantingCycleP.ListPlantingCyclesResponse, error) {
	plantingCycles, err := s.getOverdueHarvestsUsecase.Execute(ctx)
	if err != nil {
		return nil, err
	}
