...
func (s *PlantingCycleService) GetPlantingCyclesByDateRange(ctx context.Context, req *plantingCycleP.GetPlantingCyclesByDateRangeRequest) (*plantingCycleP.ListPlantingCyclesResponse, error) {
	startDate := req.StartDate.AsTime()
	endDate := req.EndDate.AsTime()

	plantingCycles, err := s.getByDateRangeUsecase.Execute(ctx, startDate, endDate)
	if err != nil {
		return nil, err
	}
